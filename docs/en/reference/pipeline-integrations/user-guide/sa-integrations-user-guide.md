...
{% include info title="Warning" content="On Windows, if Desktop is running while updating the version, a binary lock will interfere with the install and cause the installer to stop. Before this interference, some files were already copied, so this will result in an incomplete installation. To fix this, exit Desktop and select "retry"." %}
