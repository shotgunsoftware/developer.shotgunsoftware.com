...
[{% include product %}  Software](http://www.shotgunsoftware.com)'s [open source
...
When you want to access the {% include product %}  event stream, the preferred way to do so it
...
The framework is a daemon process that runs on a server and monitors the {% include product %} 
