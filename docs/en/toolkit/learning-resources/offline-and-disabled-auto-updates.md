...
2. In Shotgun, create a Pipeline Configuration entity for the project you want to lock down, with the following fields populated (In this example, we are locking down the config to use v1.0.36 of the integrations):
  ...
  ![Pipeline Configuration entity with a setup for a project with disabled updates.](./images/freeze_single_project.png)
...
![Pipeline Configuration entity with a setup for disabled updates on all projects.](./images/freeze_all_projects.jpg)
...
- Configure the exception project’s Pipeline Configuration entity to have the following field values:
    ...
    ![Two Pipeline Configurations, one disabling updates to the whole site, and the other enabling updates on a single project](./images/freeze_all_but_one_project.jpg)
