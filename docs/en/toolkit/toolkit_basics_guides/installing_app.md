...
This guide utilizes the project configuration we created in the [Editing a Pipeline Configuration](https://github.com/shotgunsoftware/developer.shotgunsoftware.com/blob/getting_started_guide/docs/en/toolkit/toolkit_basics_guides/editing_app_setting.md) guide. If you haven’t completed this guide, you can use an existing pipeline configuration and add the app there.
...
2. A centralized pipeline configuration for the identified project, or complete the [Getting Started with Configurations](link) and use the centralized configuration created in that exercise.
...
![Python app info](../../../images/learning_content/toolkit_basics_guides/installing_app/2_python_app_info.png)

The **System Name:** is the main identifier: `tk-multi-pythonconsole`. You will also need the **Latest Version:** v1.1.2.

### Confirm there’s no Python Console app in the Project environment in Maya
...
![Path to_config](../../../images/learning_content/toolkit_basics_guides/installing_app/6_path_to_config.png)

This displays the paths to the configuration files.

![Path to config 2](../../../images/learning_content/toolkit_basics_guides/installing_app/7_path_to_config2.png)
...
Wait while Shotgun copies all of the files and folders in the configuration and creates a cache for the apps. Now would be an opportune time to use an app that makes you the light, fluffy, melt in your mouth croissant to go with that perfect shot of espresso.

![Clone Complete](../../../images/learning_content/toolkit_basics_guides/installing_app/10_clone_complete.png)
...
**Step 8:** Open Shotgun Desktop and load the project you created the cloned configuration for. Select the **down arrow** in the upper right to display the configurations associated with this project and select the **Primary Clone Config 2** that you just created.

![Select clone](../../../images/learning_content/toolkit_basics_guides/installing_app/11_select_clone.png)

Shotgun Desktop is now using the cloned config for this project.

## Editing the Pipeline Configuration

**Step 9:** Find the cloned configuration in your config folder and view the `config/env/includes/app_locations.yml` file.

![Clone app location](../../../images/learning_content/toolkit_basics_guides/installing_app/12_clone_app_locations.png)
...
Shotgun Toolkit uses simple terms in [YAML](https://yaml.org/spec/1.2/spec.html) files to identify engines, settings, and what paths will lead you to them.
...
`tk-maya` is the identifier for the Maya Toolkit engine bundle of code

`@` is a Toolkit term used to denote that something is in an included file  
...
### Shotgun’s Integrations

Shotgun’s integrations are organized by bundles for apps, engines, and frameworks. The bundles are referenced in Toolkit’s YAML files using bundle identifiers. Toolkit is the platform that Shotgun’s integrations apps, engines and frameworks sit on top of.

### How the configuration references the bundles

A block with a YAML file contains settings that specify how to locate a specific bundle. 

A block within a YAML file contains a term that’s an identifier for a specific piece of code. The blocks within the YAML files that reference the code bundles contain settings that control how the identified app, engine, or framework is accessed and utilized. 

## Where to put settings for the Python Console

For this specific block in the Default Configuration, ‘tk-maya.project’ is the identifier for the Maya engine in the project environment. You want to access the Python Console app code bundle from the Maya project environment. Based on the terms used it’s a safe bet that `tk-maya.yml` is the name of the YAML file where settings for the project environment are located. Find the `settings.tk-maya.project` for `apps` in the `tk-maya.yml` file.

**Step 13:** To find the file, look for `tk-maya.yml` in the includes section above the engines.

`./includes/settings/tk-maya.yml`

**Step 14:** In the settings folder inside the includes folder, open `tk-maya.yml` and search for `settings.tk-maya.project`. Open the file in a text editor.

![tk-maya](../../../images/learning_content/toolkit_basics_guides/installing_app/13_tk-maya.png)
...
![Maya cloned config](../../../images/learning_content/toolkit_basics_guides/installing_app/14_maya_cloned_config.png)
...
![Project ID](../../../images/learning_content/toolkit_basics_guides/installing_app/16_project_ID.png)
...
Step 18: Open your terminal and browse to where your cloned configuration is stored. Change to the cloned configuration root folder `the_other_side` and run the following command: 
...
There will be a list of the available pipeline configurations that can be pushed to. Enter the ID of the primary pipeline configuration for the project you are updating the configuration for.
...
Michelles-MacBook-Air:the_other_side_clone2 micelle$

```

View the changes you made in  the cloned configuration 

Step 19: In Shotgun Desktop click on the arrow in the upper right and choose Primary in the CONFIGURATION list.

![Change config maya](../../../images/learning_content/toolkit_basics_guides/installing_app/18_change_config_maya.png)
...
The Python Console app was added to the Project environment for the chosen project. We discussed in the second guide, “[Shotgun Toolkit editing a configuration getting started guide](link), that each environment is independent, a project has a dedicated configuration, and the software integrations gather settings from the pipeline configuration when a project is loaded. For the Python Console to be available in an environment, that environment will need instructions to look in the app_locations.yml file for the location descriptor. Given this, at any point in the pipeline where you want the Python Console app to be available will need the settings that say, “use the Python Console app here.” 

Advanced Topics

One of the wonderful things about Shotgun is that anyone can create an app to use within Shotgun’s environments. Yes, even uncle Lou or your mother can create one. These apps can then be shared within the [Shotgun community](https://support.shotgunsoftware.com/hc/en-us/articles/219039828) adding to the Shotgun arsenal. If you’ve created a really stellar app you want share on the Shotgun community page email us at support@shotgunsoftware.com.

Standard Shotgun apps and apps created by the loving Shotgun community are actively discussed in the Shotgun development [Google group](https://groups.google.com/a/shotgunsoftware.com/forum/?fromgroups#!forum/shotgun-dev). The knowledge and tool base continues to grow as the Shotgun community adds their contributions.
...
![Desktop python console](../../../images/learning_content/toolkit_basics_guides/installing_app/20_desktop_python_console.png)
