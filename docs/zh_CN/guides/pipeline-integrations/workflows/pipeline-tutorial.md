...
{%include info title="注意" content="在本教程中的任何时间，您都可以通过 Maya 或 Nuke 中的 {% include product %} 菜单启动 {% include product %} 面板。在此面板中，您可以查看您的项目数据，而无需离开 DCC。它将向您显示您的当前工作上下文以及该上下文中的任何最近活动。您还可以直接在该面板中为反馈添加注释。有关详细信息，请参见 [{% include product %} 面板文档](https://support.shotgunsoftware.com/hc/zh-cn/articles/115000068574#The%20Shotgun%20Panel)。" %}
