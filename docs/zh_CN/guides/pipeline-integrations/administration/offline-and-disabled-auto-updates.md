...
- [离线使用集成](#running-the-integrations-offline)
   - [初始设置](#initial-setup)
   - [管理更新](#managing-updates)
- [禁用自动更新](#turning-off-auto-updates)
   - [在项目级别或站点级别禁用更新](#turning-off-updates-at-a-project-or-site-level)
   - [针对除一个项目以外的其他所有项目禁用更新](#turning-off-updates-for-all-but-one-project)
   - [升级](#upgrading)
...
- 如果您已接管站点配置，则系统不会检查是否有较新的 `tk-config-basic` 更新，相关详细信息[请见下文](#turning-off-updates-at-a-project-or-site-level)。
...
{% include info title="注意" content="您仍然需要能够连接到 ShotGrid 站点。 我们所说的离线是指无法连接到我们的应用商店来下载更新。" %}
...
3. 在离线计算机上设置 `SHOTGUN_BUNDLE_CACHE_FALLBACK_PATHS` 环境变量以指向此位置。
4. 在离线计算机上启动 {% include product %} Desktop 或 {% include product %} Create 后，它们将选取缓存中可用的最新升级。
...
1. 确定要使用的版本。您可以在[此处](https://community.shotgridsoftware.com/tags/c/pipeline/6/release-notes)查找集成版本。
...
如果您已经对上述示例中提到的所有项目禁用了更新，但希望对特定项目启用更新
您可以
...
2. 使用以下字段值配置例外项目的工作流配置实体：
   ...
   - 描述符(Descriptor)：`sgtk:descriptor:app_store?name=tk-config-basic`
      ![两个工作流配置，一个对整个站点禁用更新，另一个对单个项目启用更新](images/offline-and-disabled-auto-updates/freeze-all-but-one-project.jpg)
