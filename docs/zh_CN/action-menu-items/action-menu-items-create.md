...
**5. 轻量化负载(Light Payload)(light_payload)：**使用[自定义协议](https://developer.shotgridsoftware.com/zh_CN/af0c94ce/)时，您的脚本将收到用于保存有效负载信息的事件日志条目记录的 ID，而不是完整的有效负载本身。
...
**8. 以弹窗方式打开(Open in Modal Overlay)：**允许 AMI 在 iframe 窗口中打开，而不是在新选项卡中打开。请注意，如果 ShotGrid 运行的是 HTTPS，那么所有 iframe 也需要是 HTTPS。
...
**12. 访问密钥(Secret Token)：**通过设置[访问密钥](https://help.autodesk.com/view/SGSUB/CHS/?guid=SG_Administrator_ar_general_security_ar_securing_amis_html)来保护您的 AMI。
