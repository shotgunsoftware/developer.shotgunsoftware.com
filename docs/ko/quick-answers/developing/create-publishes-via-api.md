...
sgtk API는 {% include product %}에서 `PublishedFiles` 엔티티를 등록할 수 있는 [편의 메서드](https://developer.shotgridsoftware.com/tk-core/utils.html#sgtk.util.register_publish)를 제공합니다.
...
기본적으로 이 메서드는 {% include product %}에서 새로운 PublishedFile 엔티티를 생성하며, 툴킷 컨셉을 사용하여 생성을 쉽게 만들어줍니다. 여러분의 코드로 무언가를 실행하려면 다음의 행을 이용해야 할 것입니다.
