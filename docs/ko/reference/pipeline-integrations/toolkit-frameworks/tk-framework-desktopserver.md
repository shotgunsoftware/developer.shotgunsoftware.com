...
이 프레임워크는 {% include product %} 데스크톱과 함께 번들로 제공되며, [이 단계에 따라](https://developer.shotgridsoftware.com/ko/d587be80/#installation-of-desktop) 다운로드할 수 있습니다. {% include product %} 데스크톱을 실행하면 자동으로 Websocket 서버를 시작합니다.  {% include product %} 데스크톱을 이미 실행 중인 경우 설정 프로세스가 완료된 것입니다.
