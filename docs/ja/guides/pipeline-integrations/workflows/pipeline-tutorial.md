...
{% include info title="注" content="このチュートリアルで使用される単純なパイプラインはプロダクションのテストを受けていません。{% include product %} ベースのパイプラインの作成例としてのみ使用してください。" %}
...
{% include figure src="./images/tutorial/image_4.png" caption="{% include product %} で定義された規定のソフトウェア エンティティ" %}
