...
- この YAML ファイルにもフィルタ設定があります。vehicle _以外の_すべてのアセットに適用されるように `asset.yml` 内のフィルタを修正してから、タイプ vehicle のアセット_のみ_に適用されるように `asset_vehicle.yml` を修正します。[これらのフィルタの例については、こちらを参照してください](#different-file-system-layouts-for-different-pipeline-steps)。
