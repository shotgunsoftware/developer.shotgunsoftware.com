...
- 変数 `{% include product %}` には、サイトに既に接続され、[{% include product %} サイト]のデータベースや他の項目をクエリーする準備が完了した [{% include product %} API](https://developer.shotgridsoftware.com/ja/python-api/) のハンドルがあります。
...
次に、この機能についての実例を示します。Toolkit アプリの 1 つであるスナップショット アプリを見てみましょう。****
